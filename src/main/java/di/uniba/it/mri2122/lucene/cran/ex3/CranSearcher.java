...
public class CranSearcher {
    ...
    /**
     * @param args : collezione, indice, output
     */
    public static void main(String[] args) throws IOException, ParseException {
        if (args.length > 2) {
            ...
            Gson gson = new Gson();
            QueryParser qp = new QueryParser("text", new CustomAnalyzer());
            //WordnetSynonymParser qp = new WordnetSynonymParser(true, true, new CustomAnalyzer());
            int idq = 1;
            while (reader.ready()) {
                System.out.print(idq + " ");
                CranQuery query = gson.fromJson(reader.readLine(), CranQuery.class);
                String t = query.getQuery().replace("(", "").replace(")", "").replace("-", " ").replace("?", "").replace("*", "").replace("/", "").replace("aircraft", "aircraft^0.5").replace("pressure", "pressure^2").replace("panel", "panel^6").replace("flutter", "flutter^2").replace("airfoil", "airfoil^1.2").replace("magnetic", "magnetic^2").replace("derivative", "derivative^2").replace("frequency", "frequency^2").replace("flow", "flow^1.5").replace("present", "present^0.2").replace("experimental", "experimental^2"); // remove special chars
                Query lq = qp.parse(t);
